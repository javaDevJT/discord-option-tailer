...
In **Setup → Signal evaluation**, select the Codex model, reasoning effort, Fast or Standard service, and maximum chase percentage. Saving pauses the relay. The example configuration uses `gpt-6-astra`, medium reasoning, and Fast with the existing ChatGPT subscription; no API key is required. Chase controls the maximum entry premium above the alert price, independently of position sizing.
